...
def cy_host_modelaggregator(encrypted_accumulator, 
    ...
    encrypted_accumulator_num_bytes = (sum(accumulator_lengths) + len(accumulator_lengths) * (IV_LENGTH + TAG_LENGTH)) * sizeof(unsigned char)
    cdef unsigned char* c_encrypted_accumulator = <unsigned char*> PyMem_Malloc(encrypted_accumulator_num_bytes)
    if c_encrypted_accumulator is NULL:
        raise MemoryError()

    # Flatten and copy over encrypted_accumulator to a 1d array
    index = 0
    for enc_local_model in encrypted_accumulator:
        ctxt, iv, tag = enc_local_model 
        for i in range(len(ctxt)):
            c_encrypted_accumulator[index] = ctxt[i]
            index += 1  
        #  index += len(ctxt)
        for i in range(len(iv)):
            c_encrypted_accumulator[index] = iv[i]
            index += 1
        #  index += len(iv)
        for i in range(len(tag)):
            c_encrypted_accumulator[index] = tag[i]
            index += 1
        #  index += len(tag)
    ...
    cdef unsigned char* c_encrypted_old_params = <unsigned char*> PyMem_Malloc((old_params_length + IV_LENGTH + TAG_LENGTH) * sizeof(unsigned char)) 
    if c_encrypted_old_params is NULL:
        raise MemoryError()

    index = 0
    ctxt, iv, tag = encrypted_old_params
    for i in range(len(ctxt)):
        c_encrypted_old_params[index] = ctxt[i]
        index += 1
    #  index += len(ctxt)
    for i in range(len(iv)):
        c_encrypted_old_params[index] = iv[i]
        index += 1
    #  index += len(iv)
    for i in range(len(tag)):
        c_encrypted_old_params[index] = tag[i]
        index += 1
    #  index += len(tag)
    ...
    cdef unsigned char* new_params_ptr = <unsigned char*> PyMem_Malloc((old_params_length + IV_LENGTH + TAG_LENGTH) * sizeof(unsigned char))
    ...
    cdef bytes output = new_params_ptr[:new_params_length]
    cdef bytes new_iv = new_params_ptr[new_params_length:new_params_length + IV_LENGTH]
    cdef bytes new_tag = new_params_ptr[new_params_length + IV_LENGTH:new_params_length + IV_LENGTH + TAG_LENGTH]

    #  PyMem_Free(new_params_ptr[0])
    #  PyMem_Free(new_params_ptr[1])
    #  PyMem_Free(new_params_ptr[2])
    PyMem_Free(new_params_ptr)
    return output, new_iv, new_tag
